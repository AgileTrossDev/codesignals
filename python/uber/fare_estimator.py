"""
Building a Fare Estimator that can tell you how much your ride will cost before you request it. It works by passing
...
You are one of the engineers building the Fare Estimator, so knowing cost per minute and cost per mile for each car
 type, as well as ride distance and ride time, return the fare estimates for all car types.
...
cost_per_minute[i] is a positive number denoting cost per minute for the ith car type. There are at least 4 car types in
every city where Uber operates.
...
def helloWorld(name):
    ...
    return "Hello, " + name
"""
def _calc(cost_per_minute, ride_time, cost_per_mile, ride_distance):
...
def fareEstimator(ride_time, ride_distance, cost_per_minute, cost_per_mile):
    result = []
    for index,cpm in enumerate(cost_per_minute):
        result.append(_calc(cost_per_minute, ride_time,cpm,cost_per_mile[index]))
