// Note: Avoid using built-in std::nth_element (or analogous built-in functions in languages other
// than C++) when solving this challenge. Implement them yourself, since this is what you would be
...
// Displays all elements in vector
...
// Validates that Oth Element is the largest in the array
bool validate_heap(vector<int> &nums) {
  for (int i=1;i<nums.size();++i) {
   if (nums[0] < nums[i]) {
       cout << "Index: " << i << " is out of order. " << nums[0] << " " << (nums[i]) << endl;
     return false;
   }
  }
  return true;
...
void check_input(vector<int> &nums, int k) {
  cout << "INPUT SIZE: " << nums.size() << " Search Max: " << k << endl;
 // disp(nums);
  
}

void check_heap (vector<int> &heap, int input_size ) {
  //cout << "HEAP SIZE: " << heap.size() << "  Max: " << heap[0] << endl;
    if (input_size != heap.size()) cout << "SIZE ERROR IN HEAP" << endl;
    if (!validate_heap(heap)) {
      cout << "HEAP NOT VALID!" << endl;
     // disp(heap);
     // exit(-1);
    }
 // disp(nums);
...
bool validate_off_spring(vector<int> &heap, int index ){
  
  return (heap[index] >= heap[left(index)]  && heap[index] >= heap[right(index)]  );
  
}


bool validate_entire_heap (vector<int> &heap ) {
 
  for (int i=1;i<heap.size();++i) {
   if (!validate_off_spring(heap,i))   {
     cout << "Index: " << i << " have children out of order. " << heap[0] << " " << (heap[i]) << " " <<  heap[left(i)] << " " <<  heap[right(i)]   << endl;
     return false;
   }
  }
  return true;
...
// Swap two nodes in vector at index l and r
...
int get_largest_node(std::vector<int> &nums, int root) {
...
// Compares child to parent and sibling, and swaps to largest value when necessary. 
// Traverses all the way to root.  This operation does not assume parent heirachy 
// is valid, so it bubbles alway the up to the root.  One branch ending at index
// all the way up to the root will be max heapified.
void bubble_all_the_way_up(int index, std::vector<int> &nums) {
  if (index !=0) {
  
    // Find largest of parent and sibling and index.
    int parent_index =parent(index);
    int largest = get_largest_node(nums,parent_index);
    
    if (largest!= parent_index) {
      // largest node not the parent, so swap
      swap(nums,largest,parent_index);   
    }
    
    // continue bubbling until we reach the root
    bubble_all_the_way_up(parent_index,nums);
  } else {
    // index pointed to root(0), no where to bubble up
  }
}

void correct_triad(int index, std::vector<int> &nums) {
  int l = get_largest_node(nums,index);
...
void max_heapify(std::vector<int> &nums, int i) {
  int largest =get_largest_node(nums, i);
  if (largest != i) {
      swap(nums, i, largest);
...
// Bubbles up all leaf nodes in the vector input to form
// a valid max heap. 
void build_max_heap(std::vector<int> &nums) {
  ...
  // Bubble up all nodes with no chidlren, start from last node and work
  // backwards
  //for(int index = (nums.size()-1); index >=0;--index){
  //  //cout << "Working on index " << index << endl;
  //  
  //  int p = parent(index);
  //  if (nums[p] <nums[index]) swap(nums,p,index);
  //  correct_triad(index,nums);
  //  //int child_index = left(index);
  // // if (child_index >= nums.size()) {
  //    // leaf node identified, bubble it up
  //    //bubble_all_the_way_up(index,nums);
  // //   correct_triad(index)
  // // } else {
  //    // Has children,therefor is has already been bubbled up
  //  //  break;      
  ////  }   
  //}
}
...
// Assumes nums is a valid heap, except possibly the
// root node.
void bubble_down(std::vector<int> &heap, int index=0) {
  
 
  int largest = get_largest_node(heap,index);
  D cout << " LARGEST : " << largest  << " I:" << index << endl;
...
// Pops maximum value from heap and restructures heap after pop
int pop_max(std::vector<int> &nums) { 

  if (nums.size() <= 0)  {
    cout << "ERROR!!!" << endl;
    return INT_MAX;  // Empty, can't pop any more values
  }
  
  // Store the maximum value in heap 
  int ret_val = nums[0];  

  if (nums.size() > 1)  {
    // Move most recently added Node (tail) to heap root,
    // and shrink size of heap by one, effectively
    // removing perivous root
    nums[0] = nums.back();
    nums.pop_back();
    bubble_down(nums);
  } else {
    // Popping only element left in heap
    nums.pop_back(); 
  }
  
  D if (ret_val < nums[0]) cout << "ERROR in comparison" << endl;
  return ret_val;  
}
...
int kthLargestElement(std::vector<int> nums, int k) {
  // NOTE:      1 ≤ k ≤ nums.length.
  //          1 ≤ nums.length ≤ 105
  
  int input_size = nums.size();
 
  D check_input(nums, k);
  
  // Buid the heap
  build_max_heap(nums);
  
  validate_entire_heap(nums);
  
  int ret_val =0;
  for (int index = 0;index<(k);++index){
    cout << "Loop: " << index << endl;
    check_heap(nums,input_size--);
    
    ret_val = pop_max(nums);
...
void disp_res(int tc, int depth, int res){
   cout << "\n\nResult "<< tc <<"(" << depth <<"): " << res  << "\n\n\n"<<endl;
...
void test_case_1() {
  ...
  input.push_back(-123);
  disp(input);
  cout << "Result: " << (kthLargestElement(input,input.size())) << endl;
...
void test_case_2() {
  ...
  cout << "INPUT: ";
  disp(input);
  int res = (kthLargestElement(input,3));
  disp_res(1,3,res);
 
  
  res = (kthLargestElement(input,1));
  disp_res(2,1,res);
  
  res = (kthLargestElement(input,7));
  disp_res(3,7,res);
  
  res = (kthLargestElement(input,input.size()));
  disp_res(4,input.size(),res);
...
void test_case_3() {
  ...
  while(input.size() <10000)
    input.push_back(cnt++);
    
  disp(input);
  disp_res(1,0, (kthLargestElement(input,0)));
  disp_res(2,1, (kthLargestElement(input,1)));
  disp_res(2,input.size(), (kthLargestElement(input,input.size())));
...
void test_case_4() {
  // Test Case #4
    ...
    disp_res(1,918, (kthLargestElement(init(),918)));
 // disp_res(2,10000, (kthLargestElement(init(),10000)));
...
// Test the child/parent/swapping opeations
void test_tree_structure() {
  ...
  int tst = left(0);
  if (tst != 1) cout << "Failed  Left" << endl;
  
  tst = right(0);
  if (tst != 2) cout << "Failed  Right" << endl;
  
  tst = left(1);
  if (tst != 3) cout << "Failed  Left" << endl;
  
  tst = right(1);
  if (tst != 4) cout << "Failed  Right" << endl;
  
  
  tst = parent(1);
  if (tst != 0) cout << "Parent  failed" << endl;
  
   tst = parent(3);
  if (tst != 1) cout << "Parent  failed" << endl;
  
  
  tst = parent(6);
  if (tst != 2) cout << "Parent  failed" << endl;

  tst = input[0];
  int tst2 = input[1];  
  swap(input,0,1);
  if (tst != input[1] && tst2 != input[0]) cout <<"Swap failed " << endl;
  
  tst = input[2];
  tst2 = input[6];  
  swap(input,2,6);
  if (tst != input[6] && tst2 != input[2]) cout << "Swap failed " << endl;
